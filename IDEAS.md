...
- Offline buffering and backfill
...
1) NVS persistence for AppConfig (save/load/factory reset) with masked secrets in GET /config.
2) Add bearer-token auth for HTTP API (admin + read-only tokens).
3) Add Prometheus-style GET /metrics for quick monitoring.
